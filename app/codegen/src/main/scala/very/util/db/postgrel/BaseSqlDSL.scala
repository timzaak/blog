...
//
  ...
  protected def paginate[Entity](
    ...
    db.run(sql"""#$select #$where #${orderBy.map(x => s" order by $x").getOrElse("")} limit $pageSize offset ${pageSize * (page - 1)}""".as[Entity]) zip
      db.run(sql"select count(*) from #$tableName #$where".as[Long].head)
  }

  protected def select = "select #${fields} from #$tableName"

  def extractFieldNames[T: TypeTag] = typeOf[T].members.collect {
