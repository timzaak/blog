...
trait CatalogApi extends ConsulApi {
  api =>
  ...
  private[consul] trait Catalog {
    ...
    def listNodes(catalogNodesGetRequest: CatalogNodesGetRequest):Either[HttpResponse[String], List[Node]] = {
    ...
    def listServices(catalogServicesGetRequest: CatalogServicesGetRequest):Either[HttpResponse[String], Map[S,List[S]]] = {
    ...
    def listNodesForService(service:S):Either[HttpResponse[String], List[CatalogNodes4ServiceResponse]] =
      listNodesForService(CatalogNodesForServiceGetRequest(service))

    def listNodesForService(c: CatalogNodesForServiceGetRequest):Either[HttpResponse[String], List[CatalogNodes4ServiceResponse]] = {
