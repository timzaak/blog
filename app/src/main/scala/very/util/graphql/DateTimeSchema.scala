...
object DateTimeSchema {
  case object LocalDateTimeCoercionViolation
      extends ValueCoercionViolation("Date/time value expected in format [yyyy-MM-dd HH:mm:ss].")

  private[this] def parseLocalDateTime(s: String) =
  ...
  case object LocalDateCoercionViolation
      extends ValueCoercionViolation("Date value expected in format [yyyy-MM-dd].")
  ...
  implicit val localDateType = ScalarType[LocalDate](
    ...
    coerceOutput = (d, caps) =>
      if (caps.contains(DateSupport)) { java.sql.Date.valueOf(d) } else {
        d.toString
    },
    coerceUserInput = {
  ...
  case object LocalTimeCoercionViolation
      extends ValueCoercionViolation("Time value expected in format [HH:mm:ss].")
