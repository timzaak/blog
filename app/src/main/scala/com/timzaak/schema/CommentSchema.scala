...
trait CommentSchema extends CommonSchema {
  ...
  private val commentQueryType: ObjectType[GraphQLContext, CommentAction] = deriveObjectType[GraphQLContext, CommentAction](
    ObjectTypeName("commentQueryAction"),
    IncludeMethods(
      "myComments"
    ),
    DocumentField("myComments", "get my comments")
  )
